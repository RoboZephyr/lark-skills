...
计算日期范围（`since` / `until`）：
- **"上周" / 未指定 / 周报 → 上一自然 ISO 周（上周一 00:00 ~ 上周日 23:59）**（推荐 + 默认）
- "本周" → 本周一 00:00 ~ 今天（部分周）
- "最近 N 天" → N 天前 ~ 今天

> **触发节奏（已统一）**：每周一 09:00 由 launchd 自动跑 `/weekly-report 上周`，覆盖完整 ISO 上周（Mon~Sun）。周一开周会前看，节奏对齐。
> 历史上曾用过 `本周` 含义（rolling 7d），现已废弃；如果手工跑请用 `上周`。
